...
static int import(void *image, IMAGE_IMPORT_DESCRIPTOR *dirent, char *dll)
{
        ...
        void unknown_symbol_stub(void)
        {
            ...
            // Some hacks to easier find out which functions are besides the one that is missing...
            printf("%x\n", &unknown_symbol_stub);
            // get the call <address>
            int *x = __builtin_return_address(0) - 4;
            // get the address of the import table entry,...
            int *y = (int *) *x;
            warnx("> %x\n", (int *) *(y-1));
            warnx(">> %x <-- missing\n", (int *) *y);
            warnx("> %x\n", (int *) *(y+1));
            warnx("> %x\n", (int *) *(y+2));
            // couldnt we use this to lookup the name?
            __debugbreak();
        }
        ...
        for (i = 0; lookup_tbl[i]; i++) {
                ...
                if (get_export(symname, &adr) < 0) {
                        ERROR("unknw symbol: %10s:%20s -> %p", dll, symname, address_tbl[i]);
                        address_tbl[i] = (ULONG) unknown_symbol_stub;
                        continue;
                } else {
                        DBGLINKER("found symbol: %10s:%20s: addr: %p, rva = %llu",
